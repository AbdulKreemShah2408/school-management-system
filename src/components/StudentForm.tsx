  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label="Birthday" name="birthday" defaultValue={data?.birthday ? new Date(data.birthday).toISOString().split("T")[0] : ""} register={register} error={errors.birthday} type="date" />
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select className="ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("gradeId")} defaultValue={data?.gradeId}>
            {grades.length > 0 ? grades.map((grade: { id: number; level: number }) => (
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select className="ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("classId")} defaultValue={data?.classId}>
            {classes.length > 0 ? classes.map((classItem: { id: number; name: string; capacity: number; _count?: { students: number } }) => (
